$(function () {
    ...
    $(document).ready(function ($) {
        ...
        //update car
        $("#car-update-form").on("submit", function (e) {
            e.preventDefault();
            $(".modal-close-btn").click();
            updateCar();
        });

        $(".existing-cars").on("click", ".btn-delete", function () {
            var carID = $(this).attr("car-id");
            deleteCar(carID);
        });

        $(".existing-cars").on("click", ".btn-edit", function () {
            var carID = $(this).attr("car-id");
            var editModal = $("#car-edit-modal");

            blockUI();

            //fetch car details
            $.ajax({
                url: `${API_URL_ROOT}/cars?car_id=${carID}`,
                type: "GET",
                dataType: "json",
                ...
                success: function (response) {
                    ...
                    editModal.find("#car_year").val(cars[0].car_year);
                    editModal.find("#car_id").val(cars[0].car_id);
                    unblockUI();
                },
    ...
    //internall function to load all cars
    function loadCars() {
        const userID = payloadClaim(token, "user_id");
        const carsbox = $(".existing-cars");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                carsbox.html(HTMLCars);
                Alpine.initTree(document.querySelector(".existing-cars"));
                unblockUI();
            },
            error: function (req, status, error) {
                unblockUI();
    ...
    //internal function to update car
    function updateCar() {
        Swal.fire({
            title: "Attention",
            ...
            if (result.value) {
                //name vairables
                var form = $("#car-update-form"); //form
                var carID = form.find("#car_id").val();
                var fields = form.find("input.required, select.required");
    ...
    //internal function to delete a car
