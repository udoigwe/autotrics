$(function () {
    ...
    $(document).ready(function ($) {
        ...
        $("#add-car-form").on("submit", function (e) {
            ...
            addCar(); //Internal function for form submission
        });
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.error == false) {
                        ...
                        editModal
                            .find(".modal-title")
                            .text(brand.product_brand_name);
                        editModal
                            .find("#product_brand_name1")
                            .val(brand.product_brand_name);
                        editModal
                            .find("#product_brand_status")
                            .selectpicker("val", brand.product_brand_status);
                        editModal
                            .find("#product_brand_id")
                            .val(brand.product_brand_id);
                    } else {
                        showSimpleMessage(
                            "Attention",
                            response.message,
    ...
    //internal function to add new car
    function addCar() {
        Swal.fire({
            title: "Attention",
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        var actions =
                            `
                            <a href="javascript:void(0);" class="btn btn-link font-18 text-muted btn-sm btn-edit" title="Edit Product Category" data-id="` +
                            data +
                            ...
                            <a href="javascript:void(0);" class="btn btn-link font-18 text-muted btn-sm btn-delete" title="Delete Product Category" data-id="` +
                            data +
    ...
    function dataTableTwo() {
        return {
            ...
            fetchData() {
                ...
                const self = this;
                const user_id = payloadClaim(token, "user_id");
                $.ajax({
                    url: `${API_URL_ROOT}/cars?user_id=${user_id}`,
                    type: "GET",
                    headers: {
            ...
            // All your existing computed properties and methods below
            ...
            get filteredData() {
                ...
                return this.data
                    .filter(
                        (car) =>
                            car.car_model.toLowerCase().includes(searchLower) ||
                            car.car_make.toLowerCase().includes(searchLower) ||
                            car.car_milage
                                .toLowerCase()
                                .includes(searchLower) ||
                            car.car_year.toLowerCase().includes(searchLower)
                    )
                    .sort((a, b) => {
